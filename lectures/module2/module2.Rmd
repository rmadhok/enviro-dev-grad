...
\vfill
- All assignment instructions + rubrics posted
\vfill
...
- First draft expectations
    - **AT LEAST** 1 page on each section
\vfill
- See [VoxDevLit](https://voxdev.org/voxdevlit/deforestation) on Deforestation
...
\vfill
- Channel II: Technology and Infrastructure (next time)
...
- Not the right question
  - if we want to know how development affects environment, we need: 1) causality, 2) micro data, and 3) **a better definition of development**
  - e.g. manufacturing $\uparrow$ pollution, but services may not
\vfill
- A better question:
  - Which component of dev't affects which enviro outcome, through what mechanism?
...
- **Why and how** does economic development affect the environment?
  - Ex: energy, diet, self-protection, capital, institutions
\vfill
...
\vfill
- **Scope:** 4 million households treated
  - \$2.6 billion/year (half of anti-poverty budget)
  - Transfers are 1/3 of total income
  - Represents shift to new income path
\vfill 
- **Design:** Randomized pilot + non-randomized rollout
  - 1997: randomized pilot at village level
  - 1998-2000: rapid rural rollout based on **marginality** (not random)
  - Post-2000: new villages enroll, but slower
  - Within villages, household selected based on marginality index
...
$$\Delta f_{i} = \alpha + \delta T_{i} + \beta'X_{i} + \epsilon_{i}$$
  ...
  - where $\Delta f_{i}$ is \% deforested in polygon i over 2000-2003
  - $T_{i}=1$ if the locality enrolled by 2003
  - $X_{i}$ are covariates, including poverty

- But $T_{i}$ is not random!

\vfill
- **Solution:** Regression Discontinuity Design
...
- 506 villages, 320 treated (early intervention), 186 control (delayed intervention)
...
- I posted examples of assignments
\vfill
- Research proposal first draft: Oct 5th
  - If you find idea this week, gives 2 weeks to write
\vfill
...
- Development increases access to capital
...
- Continuum of agents with heterogeneous outside (non-ag) options
  - Changes in returns to ag affect number of agents who select into farming

- Farmers allocate labor and capital b/w existing land, or cultivate new land

- Let $\gamma$ denote input share allocated to cultivating new land
...
- Farmers are factor market constrained (Conning and Udry, 2007)
  - Cannot hire inputs beyond initial endowment
...
- Deforestation from GFC (Hansen et al., 2013)
  ...
  - Con: only captures loss not gain
...
- Estimate ITT since we do not know true treatment designation
\vfill
