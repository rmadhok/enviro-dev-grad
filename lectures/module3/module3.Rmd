...
- Come talk to me about your research idea
  - Write half a page every couple days
\vfill
- Start pinning down a paper to replicate
  - And think about extension (see example)
\vfill
- Next class, you have TWO options for the presentation paper
\vfill
- See [VoxDevLit](https://voxdev.org/voxdevlit/air-pollution) on air pollution!

## Environment \& Development Seminars

- [**New**] JEEP Seminar (12:30-2pm, Ruttan 119)
  - Oct 5th: Andy Hultgren (UIUC) presenting on ag. productivity and environment
  - Lunch served!

\vfill
- APEC Seminar (Fridays, 10:30-12pm, Ruttan 119)
  - Sheetal Sekhri (UVA): Sept 25th, 2026
  - Molly Lipscomb (UVA): Feb 5th, 2027
  - Adam Storeygard (Tufts): Feb 26th, 2027
  - Maulik Jagnani (Tufts): March 19th, 2027
...
- Largely from developed countries (see Janet Currie papers)
...
- Less work on non-health effects
  ...
  - Zarate-Barrera (2026): pollution and crime
\vfill
...
- Open questions:
  ...
  - \textcolor{red}{Long term effects of exposure? Consumption smoothing?}
\vfill
...
- Defensive good $q_{it}$ traded at price $P_{t}$ 
...
\vfill
- Add financial market frictions characteristic of LMICs
\vfill
...
- **Market Equilibrium**: set optimal demand for labor/capital = aggregate supply  
  - where $K_{t} = \int x^{*}_{it}di$
  - interest rates $r_t$ are determined in equilibrium as:
    $$r_t = r(K_t) $$
  - **same equilibrium logic for wages**
...
- How can we go from household model to aggregate statements about weather-health?
\vfill
...
- **Research question:** how do temperature shocks affect mortality?
  ...
  - What is the role of financial market frictions as an intermediary?
...
- Special version of binned regression: 
  - All bins $j < 80\degree$ are zero 
  - $j > 80 \degree$ bins restricted to be linearly increasing
...
## RCT Evidence: Cash Transfers and Adaptation to Extreme Heat
```{r, echo = FALSE, message = FALSE, warning = FALSE, out.width = "70%", fig.align = "center"}

knitr::include_graphics(
                        path = "assets/seminar_promo.png",
                        auto_pdf = TRUE
                        )
...
\vfill
- Monday paper: no need to fully understand WTP estimation
\vfill
- Monday lecture: print the slides and bring to class!
\vfill
- Research Proposal: first draft due in 12 days
\vfill
- Decide on a replication paper
...
- Temperature is neg. associated w/ aggregate output (Dell, Jones, and Olken 2012)
  ...
  - Effects on levels (crop output/yields) and growth rates
...
- Evidence on indirect channels: what we don't know
   - **conflict**, **institutions**, **political stability**
  - open areas for research --- see Burke, Hsiang, and Miguel (2015)

- Evidence on indirect channels: what we do know
  - crop/worker productivity: see reading list

## Somanathan et al. (2021): Temperature, Productivity, and Labor

- **Research question:** What is the effect of temperature on firm output?
...
- Workers should respond quickly when working in uncomfy temperatures
  ...
  - Distinguishes heat stress from alternative mechanisms which materialize over long time scales (i.e. conflict)
\vfill
- Two channels through which temperatures affect workers
  - Produce less (productivity)
  - Absenteeism
\vfill
- Policy: is cost-effective to invest in climate control in dev countries?
...
- Weather: public weather stations + gridded data (IMD)
  ...
  - Gridded data (1 degree) for panel data
...
$$ y = \alpha_{0} + \underbrace{\sum_{j=2}^{5} \alpha_{j}T^{j}}_\text{TFP effect} + \omega_{0} \cdot k + \underbrace{\sum_{j=2}^{5} \omega_{j}T^{j}k}_\text{effect on capital elasticity} + \beta_{0} \cdot l + \underbrace{\sum_{j=2}^{5} \beta_{j}T^{j}l}_\text{Effect on labor elasticity} $$
